...
const CardStarship = (props) => {
    ...
    return (
        <div className="card" style={{ width: "200px", backgroundColor: 'black', color: 'white' }}>
            ...
            <div className="card-body text-start">
                ...
                <Link to={`/starship-description/${props.id}`}>
                    <button id={props.id} className="btn btn-primary">Learn more</button>
                </Link>

                <button className='ms-3 btn btn-light' onClick={() => {
                   addNewFavorite()
                }}><i className={isFavorite() ? "fa-solid fa-heart text-danger" : "fa-regular fa-heart"}></i></button>
            </div>
        </div>
    )
}
